...
export default function AdministratorPage() {
  ...
  const deleteMessage = useMemo(() => {
    ...
    if (n === 0) return "";
    if (n === 1) return `정말 '${rows.find(r=>r.id===selectedIds[0])?.username}' 계정을 삭제하시겠습니까?`;
    return `정말 '${rows.find(r=>r.id===selectedIds[0])?.username}' 외 ${n - 1}개 계정을 삭제하시겠습니까?`;
  }, [selectedIds, rows]);
  ...
  return (
    <>
        ...
        <section className={clsx(CARD, "p-6 mt-6")}>
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <button type="button" className={BTN_RED} onClick={() => setOpenDelete(true)} disabled={!canDelete}>
                계정 삭제
              </button>
              <button type="button" className={BTN_YEL} onClick={() => setOpenEdit(true)} disabled={!canEdit}>
                계정 수정
              </button>
            </div>
          </div>
      ...
      {openRegister && (
        <RegisterAccountModal
          ...
          onSuccess={async (form) => {
            // 기존 모달 form: { id, password, role, desc }
      ...
      {openEdit && editRow && (
        <EditAccountModal
          ...
          initial={{
            id: editRow.username,                 
            role: editRow.name,
            desc: editRow.description ?? "",
            password: "",                          
          }}
          onSave={async (form) => {
            const payload: { password?: string; name?: string; description?: string } = {
              name: form.role,
              description: form.desc,
            };
      ...
      {/* 삭제 모달 → DELETE /api/members (body: { ids: number[] }) */}
      {openDelete && (
        <DeleteModal
          open={openDelete}
          title="계정 삭제"
        ...
        >
          <div className="space-y-2 text-center">
            ...
            <p className="text-sm text-gray-600">삭제 대상: 총 <b>{selectedIds.length}</b>개</p>
            {sampleIds.length > 0 && (
                  ...
                  {sampleIds.map((id) => (
                    <li key={id}>
                      <code className="px-1 py-0.5 bg-gray-100 rounded">
                        {rows.find(r => r.id === id)?.username ?? id}
                      </code>
                    </li>
                  ))}
                </ul>
                {selectedIds.length > sampleIds.length && (
                  <div className="mt-1 text-gray-600">… 외 {selectedIds.length - sampleIds.length}개</div>
                )}
              </div>
            )}
